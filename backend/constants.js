...
const commands = {
  runDiscovery:
    'docker run -v $(pwd)/docker/tap:/app/tap-redshift/data gbolahan/tap-redshift:b4 tap-redshift -c tap-redshift/data/config.json -d > docker/tap/catalog.json',
  runSync: (knot) => {
    if (knot) {
      return `docker run -v $(pwd)/knots/${knot}/tap:/app/tap-redshift/data --interactive gbolahan/tap-redshift:b4 tap-redshift -c tap-redshift/data/config.json --properties tap-redshift/data/catalog.json --state tap-redshift/data/state.json | docker run -v $(pwd)/knots/${knot}/target:/app/target-datadotworld/data --interactive gbolahan/target-datadotworld:1.0.0b3 target-datadotworld -c target-datadotworld/data/config.json > state.json`;
    }
  ...
  runPartialSync: (knot) => {
    ...
    return 'docker run -v $(pwd)/docker/tap:/app/tap-redshift/data --interactive gbolahan/tap-redshift:b4 tap-redshift -c tap-redshift/data/config.json --properties tap-redshift/data/catalog.json --state tap-redshift/data/state.json | docker run -v $(pwd)/docker/target:/app/target-datadotworld/data --interactive gbolahan/target-datadotworld:1.0.0b3 target-datadotworld -c target-datadotworld/data/config.json > state.json';
  }
};
const activeTargets = [
  {
